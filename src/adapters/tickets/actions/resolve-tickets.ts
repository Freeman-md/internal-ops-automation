...
export async function resolveTickets(page: Page, items: TicketItem[]): Promise<string[]> {
  ...
  for (const item of items) {
    try {
      ...
      await Promise.race([
        stateBadge.filter({ hasText: TICKET_STATES.RESOLVED }).waitFor({ timeout: 5_000 }),
        button.filter({ hasText: /no actions/i }).waitFor({ timeout: 5_000 }),
      ]);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown ticket action error";
      throw new ActionError(`Failed to resolve ticket ${item.id}`, { id: item.id, error: message });
    }
